...
1. Deploy the backend at the HTTPS origin used in `src/org-defaults.json`, or update both `src/org-defaults.json` and `manifest.json` before packaging.
...
5. Confirm the privacy policy is reachable at `https://<your-backend-domain>/privacy`.
...
3. Make sure the Chrome Web Store privacy questionnaire matches the current behavior:
   - supported pages are LinkedIn, Gem candidate, and GitHub profile pages
